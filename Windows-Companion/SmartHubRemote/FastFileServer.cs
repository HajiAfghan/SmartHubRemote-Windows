...
namespace SmartHubRemote {
    internal sealed class FastFileServer:IDisposable {
        ...
        void Receive(TcpClient tcp){try{using(tcp)using(var s=tcp.GetStream())using(var input=new BinaryReader(s,Encoding.UTF8,true))using(var output=new BinaryWriter(s,Encoding.UTF8,true)){tcp.ReceiveTimeout=30000;if(ReadJavaUtf(input)!="SMARTHUB_FILE_V1")throw new IOException("Invalid file protocol");string pin=ReadJavaUtf(input);if(pin!=currentPin()){output.Write((byte)0);WriteJavaUtf(output,"Invalid receiver PIN");output.Flush();return;}string name=Safe(ReadJavaUtf(input));long size=ReadInt64Big(input);if(size<0||size>50L*1024*1024*1024)throw new IOException("Invalid file size");string dir=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),"Downloads","SmartHub Remote");Directory.CreateDirectory(dir);string final=Unique(Path.Combine(dir,name)),part=final+"."+size+".part";long offset=File.Exists(part)?Math.Min(new FileInfo(part).Length,size):0;output.Write((byte)1);WriteInt64Big(output,offset);output.Flush();using(var file=new FileStream(part,FileMode.OpenOrCreate,FileAccess.Write,FileShare.None)){file.Position=offset;byte[] b=new byte[128*1024];long total=offset;while(total<size){int n=input.Read(b,0,(int)Math.Min(b.Length,size-total));if(n<=0)throw new EndOfStreamException("Transfer interrupted");file.Write(b,0,n);total+=n;}}File.Move(part,final);WriteJavaUtf(output,"SAVED");output.Flush();log("Fast file saved: "+final);}}catch(Exception e){log("Fast file: "+e.Message);}}
