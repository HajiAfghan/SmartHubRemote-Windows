...
namespace SmartHubRemote {
    ...
    internal sealed class RemoteServer : IDisposable {
        ...
        public void ProcessLine(string line,Action<string> send,SessionState state){try{var m=json.Deserialize<Dictionary<string,object>>(line);string type=Get(m,"type");if(!state.Paired){if(type!="pair"||Get(m,"pin")!=Pin){Send(send,new{type="error",message="Invalid pairing PIN"});return;}state.Paired=true;state.Name=Get(m,"name");Send(send,new{type="paired",server=Environment.MachineName,protocol=1});Log?.Invoke(state.Name+" paired");return;}switch(type){
            case "move":UI(()=>NativeInput.Move(Num(m,"dx"),Num(m,"dy")));break;case "click":UI(()=>NativeInput.Click(Get(m,"button")));break;case "button":UI(()=>NativeInput.Button(Get(m,"action")));break;case "scroll":UI(()=>NativeInput.Scroll((int)Num(m,"dy")));break;
            case "key":UI(()=>NativeInput.Key((int)Num(m,"code")));break;case "text":UI(()=>NativeInput.Text(Get(m,"text")));break;
            case "clipboard":UI(()=>{try{Clipboard.SetText(Get(m,"text"));}catch{}});break;
            case "screenshot":SendImage(send,"screenshot");break;case "screen_start":if(!state.Streaming){state.Streaming=true;Task.Run(()=>StreamLoop(send,state));}break;case "screen_stop":state.Streaming=false;break;
            case "pointer":UI(()=>{var b=Screen.PrimaryScreen.Bounds;Cursor.Position=new Point(b.Left+(int)(Num(m,"x")*b.Width),b.Top+(int)(Num(m,"y")*b.Height));if(Bool(m,"click"))NativeInput.Click("left");});break;
            case "launch":Launch(Get(m,"app"));break;case "system":SystemAction(Get(m,"action"));break;case "file":SaveFile(m,send);break;
        }}catch(Exception e){Send(send,new{type="error",message=e.Message});}}
