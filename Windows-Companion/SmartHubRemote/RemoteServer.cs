...
namespace SmartHubRemote {
    ...
    internal sealed class RemoteServer : IDisposable {
        ...
        public void ProcessLine(string line,Action<string> send,SessionState state){try{var m=json.Deserialize<Dictionary<string,object>>(line);string type=Get(m,"type");if(!state.Paired){if(type!="pair"||Get(m,"pin")!=Pin){Send(send,new{type="error",message="Invalid pairing PIN"});return;}state.Paired=true;state.Name=Get(m,"name");Send(send,new{type="paired",server=Environment.MachineName,protocol=5});SendCurrentClipboard(send);Log?.Invoke(state.Name+" paired");return;}Log?.Invoke("Received: "+type);switch(type){
            ...
            case "key":NativeInput.Key((int)Num(m,"code"));Ack(send,type);break;case "text":NativeInput.Text(Get(m,"text"));Ack(send,type);break;case "ping":Send(send,new{type="pong",server=Environment.MachineName,protocol=5});break;
            case "clipboard":{string clip=Get(m,"text");UI(()=>{try{if(!string.IsNullOrEmpty(clip))Clipboard.SetText(clip);}catch(Exception e){Log?.Invoke("Clipboard error: "+e.Message);}});Ack(send,type);break;}
            ...
            case "launch":{string app=Get(m,"app");RunBackground(send,"launch "+app,()=>Launch(app));break;}case "system":{string action=Get(m,"action");RunBackground(send,"system "+action,()=>SystemAction(action));break;}case "file":SaveFile(m,send);break;
        ...
        async Task StreamLoop(Action<string> send,SessionState state){while(state.Streaming){SendImage(send,"screen");await Task.Delay(300);}}
        void SendImage(Action<string> send,string type){try{lock(captureLock){var b=Screen.PrimaryScreen.Bounds;using(var full=new Bitmap(b.Width,b.Height))using(var g=Graphics.FromImage(full)){g.CopyFromScreen(b.Left,b.Top,0,0,b.Size);int w=Math.Min(type=="screenshot"?1280:1024,full.Width),h=(int)Math.Round(full.Height*(w/(double)full.Width));using(var bmp=new Bitmap(full,w,h))using(var ms=new MemoryStream()){var codec=ImageCodecInfo.GetImageEncoders().First(x=>x.FormatID==ImageFormat.Jpeg.Guid);using(var ep=new EncoderParameters(1)){ep.Param[0]=new EncoderParameter(System.Drawing.Imaging.Encoder.Quality,type=="screenshot"?68L:40L);bmp.Save(ms,codec,ep);}Send(send,new{type=type,data=Convert.ToBase64String(ms.ToArray()),width=bmp.Width,height=bmp.Height});}}}}catch(Exception e){Log?.Invoke("Screen capture: "+e.Message);Send(send,new{type="error",message="Screen capture failed: "+e.Message});}}
        ...
        void RunBackground(Action<string> send,string command,Action action){Ack(send,command);Task.Run(()=>{try{action();Log?.Invoke(command+" completed");}catch(Exception e){Log?.Invoke(command+" failed: "+e.Message);Send(send,new{type="error",message=command+" failed: "+e.Message});}});}
        void SystemAction(string action){if(action=="lock"){if(!NativeInput.LockWorkStation())throw new InvalidOperationException("Windows refused the lock command");}else if(action=="sleep"){if(!NativeInput.SetSuspendState(false,false,false))throw new InvalidOperationException("Windows refused the sleep command");}else if(action=="restart")Process.Start(new ProcessStartInfo("shutdown.exe","/r /t 10 /c \"SmartHub Remote restart\""){UseShellExecute=false,CreateNoWindow=true});else if(action=="shutdown")Process.Start(new ProcessStartInfo("shutdown.exe","/s /t 10 /c \"SmartHub Remote shutdown\""){UseShellExecute=false,CreateNoWindow=true});else throw new InvalidOperationException("Unknown system action");}
        ...
        public void SendCurrentClipboard(Action<string> send){UI(()=>{try{if(Clipboard.ContainsText()){string text=Clipboard.GetText();if(!string.IsNullOrEmpty(text))Send(send,new{type="clipboard",text=text});}}catch(Exception e){Log?.Invoke("Clipboard read: "+e.Message);}});}
